...
export default function Programmers() {
  ...
  let [idProgrammers, setIdProgrammers] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return ((dev.id === programmers ? setIdProgrammers(id) : setIdProgrammers(null)))
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idProgrammers === null ? 'royalblue' : "gold" // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { dev.id /*set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
